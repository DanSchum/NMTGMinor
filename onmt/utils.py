...
def padToBlockSizeDimOne(input, block_size):
    ...
    if input.shape[1] % block_size != 0:
        ...
        return torch.cat([input, torch.zeros((input.shape[0], padding), dtype=torch.int64).cuda()], dim=1).cuda()
...
def padToBlockSizeDimZero(input, block_size):
    if input.shape[0] % block_size != 0:
        ...
        return torch.cat([input, torch.zeros((padding, input.shape[1]), dtype=torch.int64).cuda()], dim=0).cuda()
        # Now input can be divided into full blocks
    return input
